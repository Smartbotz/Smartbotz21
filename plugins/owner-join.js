...
let handler = async (m, { conn, text, usedPrefix, command, args, participants, isOwner }) => {
  ...
  let owbot = global.owner[0] 
  m.reply(`😎 Tunggu, saya akan bergabung dengan grup`)
  try {
...
@${owbot} 
Dia adalah pencipta saya jika Anda memiliki pertanyaan
...
*${conn.user.name}* 
adalah salah satu bot multidispositif WhatsApp yang dibangun dengan Node.js, *${conn.user.name}* Menerima undangan melalui *${m.name}*

Untuk melihat Menu bot tulis

${usedPrefix}help
@${conn.user.jid.split('@')[0]} 
akan keluar secara otomatis setelahnya \n\n${msToDate(global.db.data.chats[res].expired - now)}`
