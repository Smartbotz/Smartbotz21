...
*${usedPrefix + command}*  limit [cantidad] [@user]

📌 Contoh : 
*${usedPrefix + command}* limit 65 @${m.sender.split('@')[0]}
...
`.trim()
    ...
    //conn.sendButton(m.chat, confirm, fgig, null, [['ok'], ['no']], m, { mentions: [who] })
...
handler.before = async m => {
    ...
    if (/ok?/g.test(m.text.toLowerCase())) {
...
handler.tags = ['xp']
handler.command = ['paylimit', 'transfer']
